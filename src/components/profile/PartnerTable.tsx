...
export const PartnerTable: React.FC<PartnerTableProps> = ({
  ...
  return (
    <div className="bg-zinc-900/40 border border-white/5 rounded-[3rem] p-6 sm:p-10 shadow-2xl">
      <h3 className="text-sm font-black text-white uppercase tracking-[0.2em] mb-6 sm:mb-8 flex items-center gap-3">
        <Shapes className="w-5 h-5 text-emerald-500" />
        隊友合作競爭力分析
      </h3>
      
      {/* Desktop Header */}
      <div className="hidden sm:grid grid-cols-[2fr_1fr_1fr] gap-4 pb-4 border-b border-white/5 px-2">
        <div
        ...
        <div
          className="text-[10px] font-black text-zinc-500 uppercase tracking-widest text-right cursor-pointer hover:text-white transition-colors"
      ...
      <div className="flex flex-col gap-3 mt-4 sm:mt-2">
        {teammateStats.map((s, idx) => {
          ...
          return (
            <div 
              key={idx} 
              className="group flex flex-col sm:grid sm:grid-cols-[2fr_1fr_1fr] gap-4 sm:gap-4 p-5 sm:px-2 sm:py-5 bg-white/[0.02] sm:bg-transparent hover:bg-white/[0.04] rounded-3xl sm:rounded-none sm:border-b border-white/5 transition-colors [content-visibility:auto] contain-paint items-center"
            >
              <div className="flex items-center gap-4 w-full sm:w-auto">
                <div className="w-12 h-12 sm:w-10 sm:h-10 rounded-[1.25rem] sm:rounded-xl bg-zinc-800 border border-white/10 overflow-hidden ring-offset-2 ring-offset-zinc-950 group-hover:ring-2 ring-emerald-500/50 transition-all shrink-0">
                  <img src={getAvatarUrl(tPlayer?.avatar, s.name)} className="w-full h-full object-cover" />
                </div>
                <div className="flex flex-col flex-1 min-w-0">
                  <span className="font-black sm:font-bold text-[15px] sm:text-base text-zinc-200 group-hover:text-emerald-400 transition-colors truncate">{s.name}</span>
                  <span className="sm:hidden text-[11px] font-bold text-zinc-500 mt-0.5">合作 {s.count} 場</span>
                </div>
              </div>
              ...
              <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t border-white/5 sm:border-0 self-center">
                <span className="sm:hidden text-[11px] font-black text-zinc-500 uppercase tracking-widest">共同勝率</span>
                <div className="flex flex-col items-end sm:items-end w-[60%] sm:w-auto">
                  <span className={`text-[15px] sm:text-base font-black tabular-nums ${s.winRate >= 60 ? 'text-emerald-500' : s.winRate >= 50 ? 'text-amber-500' : 'text-zinc-500'}`}>
                    {s.winRate.toFixed(1)}%
                  </span>
                  <div className="w-full sm:w-24 h-1.5 sm:h-1 bg-zinc-800 rounded-full mt-1.5 sm:mt-2 overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
