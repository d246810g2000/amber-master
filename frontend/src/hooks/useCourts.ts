...
export function useCourts({
  ...
  const toggleManualSelection = async (playerId: string) => {
    ...
    setRecommendedPlayers(d.recommended);
    // 即使不滿四人也同步到後端，確保重新整理後還在
    await syncToRemote(courts, d.recommended, {}, ["recommended"]);
  };
  ...
  const confirmWinner = async (winner: 1 | 2, score: string) => {
    ...
    try {
      ...
      await enqueueRemoteWrite(async () => {
        ...
        try {
          // A. 先寫入對戰紀錄 (這會更新資料庫並在後端 bump version)
          const recordPayload = {
          ...
          for (let attempt = 0; attempt < 3; attempt++) {
            try {
              matchResponse = await recordMatch(recordPayload);
              break;
            } catch (e) {
          ...
          // B. 接著同步場地狀態 (使用 recordMatch 回傳的新版本號，避免 VERSION_CONFLICT)
          const remoteStatus: Record<string, PlayerStatus> = {};
          // 這裡合併目前的狀態與剛剛設為 finishing 的狀態
          const mergedStatus = { ...playerStatusRef.current, ...finStatus };
          ...
          await pushState(
            ...
            { 
              silent: false, 
              enableLine: localStorage.getItem('lineNotifications') !== 'false',
              forceVersion: matchResponse?.data?.version 
            } as any
          );
