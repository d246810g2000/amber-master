...
export function GeminiBot({ players = [], playerStatus = {}, courts = [], recommendedPlayers = [] }: GeminiBotProps) {
  ...
  const displayAvatarSrc = useMemo(() => {
    ...
    return getAvatarUrl(currentUser?.picture, currentUser?.name || 'User');
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    try {
      // 1. 從後端獲取完整 AI 上下文 (包含分析過的數據)
      const ctx = await gasApi.fetchChatContext(boundPlayerId || undefined, today);
      const selfName = ctx.user?.name || currentUser?.name || '匿名球友';
...
【當前環境上下文 (由後端精確分析提供)】
- 用戶: ${selfName}
- 用戶即時戰力: ${ctx.user?.mu ? (ctx.user.mu * 10).toFixed(0) : '250'}
- 球場動態: ${ctx.today.activeCourts.map((c: any) => `${c.name}號場(${c.players.join('&')})`).join(', ') || '目前全空，建議開始排點'}
- 今日統計: 已進行 ${ctx.today.totalMatches} 場比賽，場上共有 ${ctx.today.playerCount} 位球員。

【教練準則】
1. **戰力至上**：在球場上，實力（戰力值）是唯一標準。一律稱為「戰力值」。
2. **術語限制**：嚴禁提及 "Mu", "Mu值" 或 "CP"。
3. **即時性建議**：請根據目前的球場動態提供專業的戰術分析與排隊建議。
4. **思考規範**：所有分析推導必須放在 <thought> 標籤內。
5. **行動導向**：如果球場目前全空，請主動詢問用戶是否要安排下一場對戰。
`;
