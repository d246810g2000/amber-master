...
export function LoginButton() {
  ...
  const displayName = useMemo(() => {
    if (currentUser?.isAdmin) return '專業撿球大隊長';
    if (binding?.isBound && mergedName) return mergedName;
    return currentUser?.name ?? '';
  }, [currentUser?.isAdmin, binding?.isBound, mergedName, currentUser?.name]);
  ...
  const displayAvatarSrc = useMemo(() => {
    ...
    return getAvatarUrl(currentUser?.picture, mergedName || currentUser?.name || 'Admin');
  }, [binding?.isBound, mergedAvatarStr, mergedName, currentUser?.name, currentUser?.picture]);

  const canGoToProfile = !!(currentUser?.isAdmin || (binding?.isBound && binding.playerId));
